//! Load OpenRPC inputs into a switchback [`ReferenceManual`].
...
/// Parse OpenRPC inputs and return a populated reference manual.
...
fn entry_uris_for_inputs(
    ...
    for input in inputs {
        ...
        if abs.is_file() {
            ...
            {
                let path = entry.path();
                if path.is_file() && is_openrpc_file(path) {
...
fn is_openrpc_file(path: &Path) -> bool {
    match path.file_name().and_then(|s| s.to_str()) {
        Some(name) => matches!(name, "openrpc.json" | "openrpc.yaml" | "openrpc.yml"),
        None => false,
    }
}
