//! Load OpenAPI inputs into a switchback [`ReferenceManual`].
...
/// Parse OpenAPI Description inputs and return a populated reference manual.
...
fn entry_uris_for_inputs(
    ...
    for input in inputs {
        ...
        if abs.is_file() {
            ...
            {
                let path = entry.path();
                if path.is_file() && is_openapi_file(path) {
