//! `$ref` → [`Reference`] resolution for OpenAPI populate.
...
fn name_from_pointer(pointer: &str, target_doc_uri: &str) -> Option<(&'static str, String)> {
    ...
    if pointer.starts_with("components/responses/") {
        return pointer
            .split('/')
            .nth(2)
            .map(|n| ("response", n.to_string()));
    }
    if pointer.starts_with("components/requestBodies/") {
        return pointer
            .split('/')
            .nth(2)
            .map(|n| ("request-body", n.to_string()));
    }
    if pointer.starts_with("components/securitySchemes/") {
        return pointer
            .split('/')
            .nth(2)
