//! Entry URI → OpenAPI group id for multi-entry populate.
...
pub fn entry_group_scope(entry_uri: &str, root: &Value) -> Option<String> {
    ...
    if !matches!(file_name, "openapi.yaml" | "openapi.yml" | "openapi.json") {
        return None;
    }
    let segment = path
        .parent()
...
/// Resolve the switchback group id for one OpenAPI entry document.
...
mod tests {
    ...
    fn path_convention_maps_version_segment() {
        ...
        assert_eq!(entry_group_id("v1/openapi.yaml", &root), "acme.example.v1");
    }
    ...
    fn extension_override_wins() {
        ...
        assert_eq!(entry_group_id("v1/openapi.yaml", &root), "custom.group");
    }

    #[test]
    fn non_version_openapi_yaml_uses_stem_slug() {
        let root = json!({ "info": { "title": "Tags" } });
        assert_eq!(
            entry_group_id("micro/tag-groups/openapi.yaml", &root),
            "openapi"
        );
        assert!(entry_group_scope("micro/tag-groups/openapi.yaml", &root).is_none());
    }
}
