//! Assemble a [`ReferenceManual`] from a populated AsyncAPI contract.
...
pub fn build_reference_manual(
    ...
    let manual_title = title.unwrap_or_else(|| {
        populated
            .document_title
            .clone()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| family.default_title().to_string())
    });
    ...
    let overview = resolved
        ...
        .first()
        .and_then(|uri| {
            resolved.docs.iter().find(|d| &d.uri == uri).map(|d| {
                switchback_jsonschema::envelope::Envelope::from_value(&d.value)
                    .info
                    .description
                    .clone()
                    .unwrap_or_default()
...
pub fn restore_sources(
    ...
    for doc in &manual.sources {
        ...
        std::fs::write(&out_path, &doc.content).map_err(|e| {
            switchback_traits::SwitchbackError::load(format!(
                "write restored AsyncAPI source {}: {e}",
                out_path.display()
            ))
        })?;
    }
