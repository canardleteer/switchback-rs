//! AsyncAPI fixture-driven end-to-end example: load test corpora → render mdBook projects.
//!
//! ```text
//! cargo run -p mdbook-asyncapi-example -- -o /tmp/asyncapi-books
//! cargo run -p mdbook-asyncapi-example -- --fixture streetlights-kafka -o /tmp/streetlights-book
...
#[derive(Parser)]
#[command(name = "mdbook-asyncapi")]
struct Cli {
    /// Output directory. Each fixture renders to `{output}/{fixture-id}/`.
    #[arg(short, long, default_value = "asyncapi-books")]
    output: PathBuf,
    /// Render only these fixture ids (repeatable). Default: Acme events corpus.
